...
unsigned long off;
    ...
    for (nim=0; nim<1; nim++) {
	sprintf (outfile, "%s_%d.fits", outfilename, nim);
	if (nim == 0)
		mext = 1;
	else
		mext = 0;
        if ((file_d = GF_create_img (outfile, datatype, 0, mext, amplifiers)) <= 0) {
                 printf ("no pude crear file (retruned %d", file_d);
		 goto clean;
	}
        printf ("writing image from buffer %ld\n", unsc_addr);

	if ((dref=GRTD_compute (outfile, bpp, 1,  amplifiers, namp, ncols, nrows, 0)) < 0)
		return (dref);
	pieces= 1;
	for (i=0; i<pieces; i++) {
		...
		if ((ret=GRTD_rtd (dref, amplifiers, (int)nelems/pieces, &imclosed, -1., -1.)) < 0)
               		return (ret);
		if (imclosed)
			dref = 0;
		if (i < pieces-1)
