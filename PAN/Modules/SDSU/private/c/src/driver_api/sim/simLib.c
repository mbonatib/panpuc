...
#include "sdsu_defs.h"
#include "simLib.h"
#include "diskImageLib/diskImageLib.h"
#include <errno.h>
#include <fcntl.h>
#include <sys/time.h>
#include <unistd.h>
...
int nadds=0;
int chunk=65535;
int xstart, ystart, xlen, ylen, xoverscan, xprescan;
...
{
	chunk = totpix;
	printf ("frames progress, chunk: %d\n", chunk);
	return (get_data_progress (fd));
}
...
int bpp, cols, rows;
	...
	if (artifdata == FILEIMG){
		printf ("file image\n");
		DSKIMG_GetPixels (currptr, chunk*2, &cols, &rows, &bpp);
		mapcnt = chunk;
		if (chunk > maxmappix){
			reset_buffer();
			printf ("exceeded maximum size, data overriden!!\n");
		}
		return (mapcnt);
	}
...
int ret;
	...
	if (type == FILEIMG) {
		artifdata = FILEIMG;
		if ((ret= DSKIMG_SetParams (args))<0)
			return (ret);
	}
	return (DONE);
}
