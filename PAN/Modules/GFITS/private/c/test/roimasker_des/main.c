...
init:
    ...
    if (display)
    	printf ("images will be displayed if ds9 is available\n"); 
    if (overcorr)
    	printf ("images will be line-overscan-corrected\n"); 
    if (outut)
    	printf ("images will be UT-out tagged(UTFILE)\n"); 
    while (running) {
...
//		printf ("message received: %s\n", message);
	        sscanf (message, "%*s %s %s %s", token, expid, impath);
				...
				beg = 1000*proctime.tv_sec + proctime.tv_usec/1000;
#endif
				RMASK_GetImage (impath, delimg, display, overcorr, outut);
#ifdef _TIMEDEB_
                		gettimeofday (&proctime, NULL);
				end = 1000*proctime.tv_sec + proctime.tv_usec/1000;
				printf ("proc time %ld msecs\n", end-beg);
#endif
			}
		} 
